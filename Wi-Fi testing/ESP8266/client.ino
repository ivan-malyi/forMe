...
Дальше мы реализуем клиента, который будет уметь делать отправку POST-запросом, например просто выбрав желаемый сайт, из предложенных сервером, для мониторига.
Причем сервер или клиент могут быть разными устройствами и написанными на разных языках. Для оснвоного проекта я реализую клиент на Си и сервре так же, но паралельно напишу клиент 
на Python, который так же будет подсоединяться к серверу (ESP8266).
